...
function processSuggestions(suggestions, grammar, labels) {
	var processed = [];
	var token = suggestions[0];
	for (var i in suggestions[1]) {
		var s = suggestions[1][i];
		if ((s === END) && suggestions[2]) { // the close symbol is present, and is usable
			if (token && (suggestions[2].indexOf(token) == 0)) processed.push(suggestions[2]);
			else if (!token) processed.push(suggestions[2]);
		} else { // ok, can we explain what is required?
			var poss = grammar[s][0];
			if (typeof(poss) == 'function') {
				var label = labels[s];
				if (typeof(label) != 'string') {
					for (var ind in label) { processed.push(label[ind]); };
				}
				else { processed.push(label); }; // single label case
			}
			else if (typeof(poss) == 'object') {
				for (i in poss) {
					if (token && (i.indexOf(token) == 0)) processed.push(i);
					else if (!token) processed.push(i);
				}	
			} else {
				if (token && (poss.indexOf(token) == 0)) processed.push(poss);
				else if (!token) processed.push(poss);
			}
		}
...
println('++++++++++');
CONSTRAINED_PROPERTIES['fake:thing'] = true;
var suggestions = parse('select * { x:a ');
println('Suggested: ' + suggestions);
println('++++++++++');
suggestions = parse('select * whe');
println('Suggested: ' + suggestions);
*/