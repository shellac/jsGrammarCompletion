...
var sparql = {
	...
	type: [ 'rdf:type', [ 'constklass', 'klass' ] ],
...
function processSuggestions(suggestions, grammar, labels) {
	...
	for (var i in suggestions[0]) {
		...
		if ((s == EMPTY) && suggestions[1]) { // the close symbol is present, and is usable
			...
			p = grammar[s][0];
			if (typeof(p) == 'function') processed.push(labels[s]);
			else if (typeof(p) == 'object') {
				for (i in p) processed.push(i);
			} else processed.push(p);
		}
...
/*
parse('select ?foo ?bar { a:a b:b c:c . }');
println('++++++++++');
parse('select ?foo ?bar where { optional { a:a b:b c:c , d:d . } }');
println('++++++++++');
parse('select distinct ?foo ?bar { { ?a b:b c:c ; d:e ?f . } }');
println('++++++++++');
println('Suggested: ' + parse('select * { ?a '));
println('Suggested: ' + parse('select '));
*/
