...
var sparql = {
	...
	subject: [ /:.*/, [ 'predicate', 'a', 'type' ] ],
	a: [ 'a', [ 'klass' ] ],
	type: [ 'rdf:type', [ 'klass' ] ],
	klass: [ /:.*/, [ 'comma', 'semi', 'stop' ] ],
	predicate: [ /:.*/, [ 'object' ]],
...
function suggest(tokens, grammar) {
	...
	if (suggestions) return processSuggestions(suggestions, grammar);
}

/* Turn next stage of parser into an array of useful hints */
function processSuggestions(suggestions, grammar) {
	var processed = [];
...
function match(token, object) {
	if (typeof(object) == 'string') return object == token;
